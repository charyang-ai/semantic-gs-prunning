...
def evaluate(model_paths):
    ...
    for scene_dir in model_paths:
        try:
            ...
            per_view_dict_polytopeonly[scene_dir] = {}  # ✅ 修复空格
            ...
            for method in os.listdir(test_dir):
                ...
                for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                    # ✅ 修复：统一在掩码区域计算所有指标
                    render_target = renders[idx][masks[idx]][None, ...]
                    gt_target = gts[idx][masks[idx]][None, ...]
                    
                    o_ssim = ssim(render_target, gt_target)
                    o_psnr = psnr(render_target, gt_target)
                    o_lpips = lpips(render_target, gt_target, net_type='vgg')
                    
                    # AVG 计算 (几何平均)
                    o_avg = torch.exp(torch.log(torch.tensor([
                        10**(-o_psnr / 10), 
                        math.sqrt(1 - o_ssim), 
                        o_lpips
                    ])).mean())
                    ...
                    avgs.append(o_avg.item())  # ✅ 添加.item()
